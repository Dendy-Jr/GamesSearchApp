...
class GamesFragment :
        ...
        gamesAdapter = GamesAdapter(
            ...
            object : ClickListener<Int> {
                override fun click(item: Int) {
                    val directions =
                        GamesFragmentDirections.actionGamesFragmentToDetailFragment(item)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.action_favorites) {
            findNavController().navigate(GamesFragmentDirections.actionGamesFragmentToFavoritesFragment())
            return true
        }
