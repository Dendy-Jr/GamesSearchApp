...
) : Abstract.Mapper.DetailCloudObject {
    override fun <T> map(mapper: DetailCacheMapper<T>) = mapper.map(
        ...
        genre = genre,
        id = id,
        systemRequirements = systemRequirements?.map(BaseSystemRequirementsCacheMapper())
            ?: SystemRequirementsCache.EMPTY,
        platform = platform,
        publisher = publisher,
        releaseDate = releaseDate,
        screenshots = screenshots?.map { it.map(BaseScreenshotCacheMapper()) } ?: emptyList(),
